...
cdc_mortality |>
  ...
  mutate(`Non-firearm` = All-Firearm) |>
  ...
  geom_text(data = ~ . |> 
              filter((type %in% c("All", "Firearm", "Non-firearm") & year == 1982)) |> 
              mutate(rate = ifelse(type %in% c("Firearm", "Non-firearm"), rate - 1.5, rate  + 1)), 
            aes(label = type), 
            family = plot_font,
            size = 6) +
  scale_y_continuous(breaks = c(0,3,6,9), labels = c(" 0", " 3", " 6", " 9"), expand = expansion(c(0,0.1)), limits = c(0,12)) +
...
chicago_homicide_rates |>
  ...
  ggplot(aes(x = year, y = rate, group = type, color = type, linetype = type)) + 
  geom_line(linewidth = 0.75) +
  scale_y_continuous(breaks = c(0,10,20,30), expand = expansion(c(0,0.1)), limits = c(0,35)) +
  geom_text(data = ~ . |> 
              filter((year == 1983 & type != "Firearm" ) | (type == "Firearm" & year == 1985)) |> 
              mutate(rate = ifelse(type == "Non-firearm", rate -3, rate + 4)), aes(label = type), family = plot_font, size = 6) +
...
## Homicide in similar systems {background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.2"}
...
unodc_homicide |>
  ...
  filter(country %in% c("United States", "United Kingdom", "Canada", "Czechia", "Germany") & !is.na(hom_rate)) |>
  mutate(country = factor(country, 
                          levels = c("United States", "Canada", "United Kingdom","Czechia", "Germany"),
                          labels = c("US", "Canada", "UK", "Czechia", "Germany"))) |>
  ggplot(aes(x = year, y = hom_rate, color = country)) +
    ...
    geom_text(data = ~ filter(.x, year == max(year), .by = country) |>
              mutate(hom_rate = case_when(
                country == "UK" ~ hom_rate*1.18, 
                country == "Germany" ~ hom_rate *1.1, 
                TRUE ~ hom_rate)), 
...
## Homicide high violence countries {background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.2"}
...
unodc_homicide |>
  select(country = Entity,
         ...
         year = Year,
         hom_rate = `Homicide rate per 100,000 population - sex: Total - age: Total`) |>
  filter(country %in% c("United States",  "Mexico", "Brazil", "Philippines", "Pakistan") & !is.na(hom_rate)) |>
  mutate(country = factor(country, 
                          levels = c("United States", "Mexico", "Brazil", "Philippines", "Pakistan"),
                          labels = c("US", "Mexico", "Brazil",  "Philippines", "Pakistan")),) |>
  ggplot(aes(x = year, y = hom_rate, color = country)) +
  geom_line(aes(linetype = country), linewidth = 0.75) +
  geom_text(data = ~ filter(.x, year == max(year), .by = country) |>
              mutate(hom_rate = case_when(
                country == "United States" ~ hom_rate + 0.6,
                country == "Philippines" ~ hom_rate + 0.45, 
                country == "Pakistan" ~ hom_rate - 0.45,
                TRUE ~ hom_rate)), 
            aes(x = 2023.5, label = country), 
            hjust = 0) +
  scale_x_continuous(limits = c(1990, 2030), 
...
## Age of handgun homicide offenders {background-image="img/pedro-lastra-5Bv3EcijAl4-unsplash.jpg" background-opacity="0.2"}
...
col_pal <- c("#7CAE00", "#C77CFF", "grey40")
col_vec <- setNames(col_pal, c("Adolescence", "Adulthood", ""))

shr_props_year_std <- shr_props_year_std |>
  mutate(stage = ifelse(stage != "Adolescence", "Adulthood", stage))

shr_sums <- shr_props_year_std |> 
  summarize(pr = sum(pr),
            label = str_c(100*round(sum(pr),2), "%"),
            .by = c(year, stage)) |>
  filter(stage != "") |>
  mutate(x = ifelse(stage == "Adolescence", 8, 34),
         x = ifelse(year == 2021 & stage == "Adulthood", x + 4, x),
         y = 0.04,
         label = str_replace(str_c(label, " in ", str_to_lower(stage)), " ad", "\nad"))

# Get median offender age
shr_medians <- shr_props_year_std |>
  mutate(cum_pr = cumsum(pr), .by = year) |>
  group_by(year) |>
  filter(cum_pr >= 0.5) |>
  slice_head(n=1) |>
  mutate(yend = pr + 0.02 + (year==1995) * 0.017) |> 
  select(year, age, pr, yend) |>
  mutate(label = str_c("Median age: ", age))

two_period_age_plot <- shr_props_year_std |>
  filter(age <= 60) |>
  ggplot(aes(x = age, y = pr)) + 
  geom_col(aes(color = stage, fill = stage), linewidth = 0.2) +
  geom_text(data = shr_sums, 
            aes(x = x, y = y, label = label, color = stage),  size = text_size*1.5, family = plot_font, lineheight = 1) +
  geom_segment(data = shr_medians, aes(x = age, xend = age, yend =yend), y = 0, color = "white", linetype = "dashed", inherit.aes = FALSE, linewidth = 0.5) +
  geom_text(data = data_frame(age = c(50, 50), pr = c(0.066, 0.066), year = c(1995, 2021)),
                              aes(x = age, y = pr, label = year), color = "white", size = text_size*2) +
  geom_text(data = shr_medians, aes(x = age, y = yend + 0.002, label = label), 
            size = text_size*1.5, color = "white", inherit.aes = FALSE) +
  scale_fill_manual(values = col_vec) +
  coord_cartesian(xlim = c(5, 62), ylim = c(0, 0.078), clip = "off")  +
  scale_color_manual(values = col_vec) +
  scale_x_continuous(breaks = seq(10, 80, by = 10), expand = c(0,0), labels = c(seq(10, 80, by = 10))) +
  scale_y_continuous(breaks = seq(0, 0.08, by = 0.02), expand = c(0,0)) +
  facet_wrap(~year, strip.position = "top", nrow = 1, label = labeller(year = c("1995" = "Year: 1995", "2021" = "Year: 2021"))) +
...
year_lines <- tribble(~x, ~xend, ~y, ~yend,
        1995, 1995, 0, 0.42,
        2021, 2021, 0, 0.42)

shr_adolescent_prop_plot <- shr_props_year |>
  summarize(pr = sum(pr), .by = c(stage, year)) |>
  filter(stage == "Adolescence") |>
  ggplot(aes(x = year, y = pr)) + 
  geom_line(color= "white") +
  geom_segment(data = year_lines, aes(x = x, xend = xend, y = y, yend = yend), color = "white", linetype = "dashed", inherit.aes = FALSE, linewidth = 0.5) +
  geom_text(data = year_lines, aes(x = x, y = yend + 0.03, label = x), 
            size = text_size*2, color = "white", inherit.aes = FALSE) +
  scale_y_continuous(limits = c(0,0.5), expand = FALSE) +
  theme_minimal(base_size = 16) +
  theme(panel.grid = element_blank(),
...
( two_period_age_plot / shr_adolescent_prop_plot) + plot_layout(heights = c(1, 0.6)) &
...
```{r sex-differences}
load("./data/derived/cdc_monthly_sex.RData")
load("./data/derived/cdc_yearly_sex.RData")


base_size <- 13

full_period_plot <- ggplot(cdc_yearly_sex |>
         filter(year >= 1980), aes(x = year, y = rate, color = type, linetype = type)) + 
  facet_wrap(~sex, nrow = 2, scales = "free_y", axes = "all", 
             labeller = labeller(sex = c("Male" = "Male by year 1980–2023", 
                                          "Female" = "Female by year 1980–2023")) ) +
  
  geom_text(data = ~ .x |>
              select(year, type, sex, rate)   |> filter(year == 1985) |> 
                          mutate(rate = case_when(
                            type == "All" ~ rate*1.15, 
                            type == "Firearm" ~ rate*0.85,
                            type == "Non-firearm" & sex == "Female" ~ rate*1.2,
                            type == "Non-firearm" & sex == "Male" ~ rate*0.8)),
            size = 3,
            aes(label = type), 
            family = plot_font) +
  annotate("rect", xmin = 2013, xmax = 2023, ymin = 0, ymax = Inf, fill = NA, color = "grey50", linetype = "dotted") +
  geom_line() +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = NULL, y = "Homicides rate per 100,000", linetype = "Type") +
  theme_classic(base_size = base_size)

full_period_prop_plot <- cdc_yearly_sex |>
  pivot_wider(names_from = type, values_from = rate) |>
  mutate(prop = Firearm/All) |>
  ggplot(aes(x = year, y = prop, color = sex, linetype = sex)) +
  annotate("rect", xmin = 2013, xmax = 2023, ymin = 0, ymax = Inf, fill = NA, color = "grey50", linetype = "dotted") +
  geom_line() +
  scale_linetype_manual(values = c("Male" = "solid", "Female" = "longdash")) +
  scale_y_continuous(limits = c(0,1)) +
  geom_text(data = ~ .x |>
              select(year, sex, prop)   |> filter(year == 1985) |> 
              mutate(prop = ifelse(sex == "Male", prop*1.15, prop*0.85)),
            size = 4,
            aes(label = sex), 
            family = plot_font) +
  labs(x = NULL, y = "Proportion", title = "By year 1980–2023") +
  theme_classic(base_size = base_size + 2)



late_period_plot <- 
  ggplot(cdc_monthly_sex, aes(x = date, y = deaths, color = type, linetype = type)) + 
  # geom_vline(xintercept = as.Date("2020-03-01"), linetype = "solid", alpha = 0.5, linewidth = 0.25) +
  # geom_vline(xintercept = as.Date("2020-05-01"), linetype = "solid", alpha = 0.5, linewidth = 0.25) +
  geom_vline(xintercept = as.Date("2020-05-01"), linetype = "dotted") +
  geom_line() +
  geom_text(data = ~ .x |>
              select(date, type, sex, deaths) |>
              filter(date == as.Date("2014-01-01")) |> 
              mutate(deaths = case_when(
                type == "All" & sex == "Male" ~ deaths*1.3, 
                type == "Firearm" & sex == "Male" ~ deaths*0.7, 
                type == "All" & sex == "Female" ~ deaths*1.15, 
                type == "Firearm" & sex == "Female" ~ deaths*1.4, 
                type == "Non-firearm" ~ deaths*0.75,
                TRUE ~ NA_real_)), 
            size = 3,
            aes(label = type), 
            family = plot_font)  +
  facet_wrap(~sex, nrow = 2, scales = "free_y", axes = "all", labeller =
               labeller(sex = c("Male" = "Male by month 2013–2023",
                                 "Female" = "Female by month 2013–2023"))) +
  scale_x_date(date_labels = "%Y", date_breaks = "2 years") + 
  scale_y_continuous(limits = c(0, NA)) +
  # scale_y_log10(limits = c(100, 2500), breaks = c(125, 250, 500, 1000, 2000)) +
  labs(x = NULL, y = "Homicide count", linetype = "Type", color = "Sex") +
  theme_classic(base_size = base_size) 


late_period_prop_plot <- cdc_monthly_sex |>
  pivot_wider(names_from = type, values_from = deaths) |>
  mutate(prop = Firearm/All) |>
  ggplot(aes(x = date, y = prop, color = sex, linetype = sex)) + 
  geom_vline(xintercept = as.Date("2020-05-01"), linetype = "dotted", color =  "grey50") +
  geom_line() +
  scale_linetype_manual(values = c("Male" = "solid", "Female" = "longdash")) +
  scale_y_continuous(limits = c(0,1)) +
  geom_text(data = ~ .x |>
              select(date, sex, prop)   |> filter(date == as.Date("2014-01-01")) |> 
              mutate(prop = ifelse(sex == "Male", prop*1.1, prop*0.8)),
            size = 4,
            aes(label = sex), 
            family = plot_font) +
  labs(x = NULL, y = "Proportion", title = "By month 2013–2023") +
  theme_classic(base_size = base_size + 2)


combined_plot <- full_period_plot + late_period_plot &   
  theme(panel.grid = element_blank(),
        panel.grid.major.y = element_line(color = "grey50", linewidth = rel(0.25)),
        strip.background = element_blank(),
        strip.text = element_text(family = plot_font,  color = "white", size = rel(1), margin = margin(0,0,2,0), hjust = 0),
        plot.margin = margin(6,2,2,2, "pt"),
...
combined_plot <- combined_plot &  plot_annotation(theme = theme(plot.background = element_rect(fill = "transparent",colour = NA), text = element_text(family = plot_font,  color = "white")))
combined_plot
```
...
```{r}
load("./data/derived/cdc_monthly_race.RData")
load("./data/derived/cdc_yearly_race.RData")

base_size <- 13

full_period_plot <- 
  ggplot(cdc_yearly_race|>
         filter(year >= 1980), aes(x = year, y = rate,  linetype = type, color = type),
         aes(x = year, y = rate, color = type, linetype = type)) + 
  facet_wrap(~race, nrow = 2, scales = "free_y", axes = "all", 
             labeller = labeller(race = c("Black" = "Black by year 1980–2023", 
                                          "White" = "White by year 1980–2023")) ) +
  geom_text(data = ~ .x |>
              select(year, type, race, rate)   |> filter(year == 1984) |> 
              mutate(rate = case_when(
                type == "All" ~ rate*1.15, 
                type == "Firearm" ~ rate*0.91,
                type == "Non-firearm" ~ rate*0.8)),
            size = 3,
            aes(label = type), 
            family = plot_font) +
  annotate("rect", xmin = 2013, xmax = 2023, ymin = 0, ymax = Inf, fill = NA, color = "grey50", linetype = "dotted") +
  geom_line() +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = NULL, y = "Homicide rate per 100,000", linetype = "Type") +
  theme_classic(base_size = base_size)



full_period_prop_plot <- cdc_yearly_race |>
  pivot_wider(names_from = type, values_from = rate) |>
  mutate(prop = Firearm/All) |>
  ggplot(aes(x = year, y = prop, color = race, linetype = race)) +
  annotate("rect", xmin = 2013, xmax = 2023, ymin = 0, ymax = Inf, fill = NA, color = "grey50", linetype = "dotted") +
  geom_line() +
  scale_linetype_manual(values = c("Black" = "solid", "White" = "longdash")) +
  scale_y_continuous(limits = c(0,1)) +
  geom_text(data = ~ .x |>
              select(year, race, prop)   |> filter(year == 1985) |> 
              mutate(prop = ifelse(race == "Black", prop*1.15, prop*0.85)),
            size = 4,
            aes(label = race), 
            family = plot_font) +
  labs(x = NULL, y = "Proportion", title = "By year 1980–2023") +
...
late_period_plot <- 
  ggplot(cdc_monthly_race , aes(x = date, y = deaths, linetype = type, color = type)) +
  geom_vline(xintercept = as.Date("2020-05-01"), linetype = "dotted") +
  geom_line() +
  geom_text(data = ~ .x |>
              select(date, type, race, deaths) |>
              filter(date == as.Date("2014-01-01")) |> 
              mutate(deaths = case_when(
                type == "All" & race == "Black" ~ deaths*1.4, 
                type == "Firearm" & race == "Black" ~ deaths*0.65, 
                type == "All" & race == "White" ~ deaths*1.15, 
                type == "Firearm" & race == "White" ~ deaths*1.3, 
                type == "Non-firearm" ~ deaths*0.7,
                TRUE ~ NA_real_)), 
            size = 3,
            aes(label = type), 
            family = plot_font)  +
  facet_wrap(~race, nrow = 2, scales = "free_y", axes = "all", labeller =
               labeller(race = c("Black" = "Black by month 2013–2023",
                                 "White" = "White by month 2013–2023"))) +
  scale_x_date(date_labels = "%Y", date_breaks = "2 years") + 
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = NULL, y = "Homicide count", linetype = "Type", color = "Race") +
  theme_classic(base_size = base_size) 


late_period_prop_plot <- cdc_monthly_race |>
  pivot_wider(names_from = type, values_from = deaths) |>
  mutate(prop = Firearm/All) |>
  ggplot(aes(x = date, y = prop, color = race, linetype = race)) + 
  geom_vline(xintercept = as.Date("2020-05-01"), linetype = "dotted", color =  "grey50") +
  geom_line() +
  scale_linetype_manual(values = c("Black" = "solid", "White" = "longdash")) +
  scale_y_continuous(limits = c(0,1)) +
  geom_text(data = ~ .x |>
              select(date, race, prop)   |> filter(date == as.Date("2014-01-01")) |> 
              mutate(prop = ifelse(race == "Black", prop*1.1, prop*0.8)),
            size = 4,
            aes(label = race), 
            family = plot_font) +
  labs(x = NULL, y = "Proportion", title = "By month 2013–2023") +
...
> This is mirrored in Sampson and Bartusch’s (1998: 782) normative conception of legal cynicism as a state in which the (legal) **rules of society are no longer seen as binding** within a community: “Normlessness and powerlessness tend also to go hand in hand, breeding **cynicism about the rules of the society and their application**”.

:::

::: {.column width="45%"}

![](img/sampson-bartusch.png)
...
# A research agenda {background-image="img/benjamin-suter-mpLex62zVKQ-unsplash.jpg" background-opacity="0.4"}
...
## Early evidence {background-image="img/benjamin-suter-mpLex62zVKQ-unsplash.jpg" background-opacity="0.2"}
...
## New worries {background-image="img/benjamin-suter-mpLex62zVKQ-unsplash.jpg" background-opacity="0.2"}
